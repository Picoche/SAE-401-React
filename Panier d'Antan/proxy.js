...
app.get("/boutiques/places/details", async (req, res) => {
  const url = `https://maps.googleapis.com/maps/api/place/details/json?place_id=${req.query.place_id}&fields=formatted_address,address_components,name,rating,user_ratings_total,reviews,opening_hours,name,geometry,photos,formatted_phone_number&key=AIzaSyDPpCouT8a5CIliE6YhC3tJ4we32-jy6vY&language=fr&types=establishment`;
  const response = await fetch(url);
  const json = await response.json();
  console.log(json);
  console.log(url);
